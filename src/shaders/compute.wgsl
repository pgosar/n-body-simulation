...
fn main(@builtin(global_invocation_id) global_invocation_id: vec3<u32>) {
    ...
    if gpu_info.motion > 0.0 {
        ...
        for (var j: u32 = 0u; j < u32(gpu_info.particles); j = j + 1u) {
            ...
            temp = temp + (normalize(diff) * (dataOld.old[j].mass) / (length2(diff) + dataOld.old[j].calibrate));
        }
        dataCurrent.data[i].vel = dataCurrent.data[i].vel + vec3<f32>(temp * G * gpu_info.motion);
        dataCurrent.data[i].pos = dataCurrent.data[i].pos + dataCurrent.data[i].vel * gpu_info.motion;
    }
}
